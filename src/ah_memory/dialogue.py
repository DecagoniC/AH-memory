...
class DialogueAgent:
    ...
    def talk_stream(self, user_text: str, ticks: int = 6) -> Iterator[dict[str, Any]]:
        ...
        ask_ticks_n = ticks if user_perc.kind == "question" else min(ticks, 2)

        yield {"event": "status", "phase": "activation"}
        mem = self._memory_context(user_text)
        i_ask = len(ign.traces)
        ask = self.agent.ask(user_text, ticks=ask_ticks_n, perception=user_perc)
